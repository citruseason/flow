...
**CORE Document Requirement:** Every rule MUST reference at least one CORE document via an `upstream:` field (e.g., `- **Upstream:** harness/PRODUCT.md#architecture`). When creating or updating rules, identify the relevant source document — CORE documents (`harness/ARCHITECTURE.md`, `harness/PIPELINE.md`, `harness/OBSERVABILITY.md`, `harness/SECURITY.md`), rule files (`harness/rules/*.md`), or `harness/PRODUCT.md` — and section that justifies the rule. Rules without an `upstream:` reference are incomplete and must not be finalized.
